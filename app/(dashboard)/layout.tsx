...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  .eq('user_id', uid),
      ...
      setUsage({ rules: rulesCount || 0, templates: templatesCount || 0, orders: ordersCount || 0 })
      setLoading(false)
    })
  }, [])
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#F7F7F7', fontSize: 13, color: '#9CA3AF', fontFamily: "'DM Sans', sans-serif" }}>
    ...
    <>
      {plan === 'free' && (
        <div className="upgrade-card">
          ...
          <Link href="/pricing" className="upgrade-btn">Upgrade — 7-day free trial →</Link>
          <p style={{ fontSize: 10, color: '#9CA3AF', textAlign: 'center', marginTop: 6 }}>No credit card required</p>
        </div>
      )}
      {plan === 'starter' && (
        <div style={{ background: '#F5F3FF', border: '1.5px solid #DDD6FE', borderRadius: 12, padding: '12px 14px', marginBottom: 10 }}>
          ...
          <p style={{ fontSize: 11, color: '#7C3AED', margin: '0 0 8px', lineHeight: 1.5 }}>Unlimited rules, unlimited AI + Priority support.</p>
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .limit-bar-track { height:4px; border-radius:4px; background:#F3F4F6; margin-top:4px; overflow:hidden; }
        .limit-bar-fill  { height:100%; border-radius:4px; background:#EE4D2D; transition:width .3s; }
        ...
        .mobile-header {
          ...
          height:54px; background:#fff; border-bottom:1px solid #F3F4F6;
        ...
        .mobile-nav-btn:active { background:#FFF1EE; }
        .mobile-nav-btn span { font-size:9px; font-weight:600; color:#9CA3AF; }
        .mobile-nav-btn.active span { color:#EE4D2D; }

        /* ── Drawer ── */
        .drawer-overlay { position:fixed; inset:0; z-index:100; background:rgba(0,0,0,0.4); animation:fadeIn .2s ease; }
        .drawer {
        ...
        .hamburger:active { background:#FFF1EE; }
        ...
        <>
          ...
          <div className="drawer">
            <div style={{ padding:'16px', borderBottom:'1px solid #F3F4F6', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              <Link href="/dashboard" style={{ display:'flex', alignItems:'center', gap:8, textDecoration:'none' }} onClick={() => setDrawerOpen(false)}>
                <div style={{ width:28, height:28, borderRadius:7, background:'#EE4D2D', display:'flex', alignItems:'center', justifyContent:'center', fontSize:14 }}>📬</div>
                <div>
              ...
              <button onClick={() => setDrawerOpen(false)} style={{ background:'#F3F4F6', border:'none', borderRadius:8, width:32, height:32, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>
            ...
            <div style={{ padding:'10px' }}><UpgradeSection /></div>
            <div style={{ flex:1 }} />
            <div style={{ padding:'10px', borderTop:'1px solid #F3F4F6' }}>
              <div style={{ display:'flex', alignItems:'center', gap:9, padding:'9px 10px', borderRadius:9, background:'#F9FAFB', marginBottom:4 }}>
                <div style={{ width:28, height:28, borderRadius:8, background:pc.bg, color:pc.color, display:'flex', alignItems:'center', justifyContent:'center', fontSize:12, fontWeight:700, flexShrink:0 }}>{userInitial}</div>
                <div style={{ minWidth:0, flex:1 }}>
                  <div style={{ fontSize:12, fontWeight:600, color:'#111', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{userEmail}</div>
      ...
      <div style={{ display:'flex', minHeight:'100vh', fontFamily:"'DM Sans',sans-serif" }}>
        <div className="desktop-sidebar" style={{ width:224, background:'#fff', borderRight:'1px solid #F3F4F6', display:'flex', flexDirection:'column', flexShrink:0, position:'sticky', top:0, height:'100vh', overflowY:'auto' }}>
          ...
          <div style={{ padding:'10px 10px 0' }}><UpgradeSection /></div>
          <div style={{ flex:1 }} />
          <div style={{ padding:'10px', borderTop:'1px solid #F3F4F6' }}>
            <div style={{ display:'flex', alignItems:'center', gap:9, padding:'9px 10px', borderRadius:9, background:'#F9FAFB', marginBottom:4 }}>
              <div style={{ width:28, height:28, borderRadius:8, background:pc.bg, color:pc.color, display:'flex', alignItems:'center', justifyContent:'center', fontSize:12, fontWeight:700, flexShrink:0 }}>{userInitial}</div>
              <div style={{ minWidth:0, flex:1 }}>
                <div style={{ fontSize:12, fontWeight:600, color:'#111', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis', maxWidth:120 }}>{userEmail}</div>
                <div style={{ display:'flex', alignItems:'center', gap:4, marginTop:1 }}>
                  <span style={{ width:5, height:5, borderRadius:'50%', background:pc.dot, display:'inline-block' }} />
                  <span style={{ fontSize:10, color:pc.color, fontWeight:600 }}>{pc.label}</span>
                </div>
      ...
      <nav className="mobile-bottom-nav">
        {navItems.map(item => {
          const active = pathname === item.href
          return (
            <Link key={item.href} href={item.href} className={`mobile-nav-btn${active ? ' active' : ''}`}>
              <div style={{ position:'relative' }}>
                {item.icon(active)}
                {item.badge && item.badge > 0 && (
                  <span style={{ position:'absolute', top:-4, right:-6, background:'#EE4D2D', color:'#fff', fontSize:9, fontWeight:700, padding:'1px 4px', borderRadius:20, lineHeight:1.4 }}>
                    {item.badge}
                  </span>
                )}
              </div>
              <span>{item.label}</span>
              {active && <span style={{ position:'absolute', bottom:6, width:4, height:4, borderRadius:'50%', background:'#EE4D2D' }} />}
            </Link>
          )
        })}
      </nav>
